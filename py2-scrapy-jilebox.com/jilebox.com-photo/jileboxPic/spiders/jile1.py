    ...
    def PutUrl(self):
        with ThreadPoolExecutor(5) as executor:
            ...
            for i in range(start_num, end_num):
                executor.submit(self.PutPicUrl, i)


    def PutPicUrl(self,i):
        time.sleep(10)
        print '线程池执行'
        pic_list_api = 'http://www.jimobox.com/api/v1.1/?page=' + str(i)
        ...
        for each in retjson1['data']['items']:
            photo_page_url='http://www.jimobox.com/photo/{}'.format(each['id'])
            ...
            for pic_url in pic_urls:
                ...
                retdata=select_down_info(tdata)
...
class CDownload ():   # 继承父类threading.Thread
    def __init__(self,path):
        self.path=path
        self.video_md5_file='D:\图片4：md5记录集\图片md5记录集.db'.decode('utf-8')

    def set_video_md5_file(self,video_md5_file):
        self.video_md5_file=video_md5_file

    def jpg_download(self):
        while True:
            global q
            down_info=q.get()
...
class JileSpider(scrapy.Spider):
    ...
    def parse(self, response):
        ...
        for j in range(5):  # 新建5个线程 等待队列
            down = CDownload('D:\图片1：待入库\\'.decode('utf-8'))
            t3 = Thread(target=down.jpg_download)
            t3.setDaemon(True)
            t3.start()
